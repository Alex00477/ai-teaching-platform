...
当前处于需求和技术方案收敛阶段，尚未创建可运行的前后端源码、数据库迁移或部署文件。原始需求保留在 [需求规约.md](需求规约.md)，已确认的产品和工程取舍保留在 [decisions/](decisions/)。
...
当前 API 只提供 `/healthz` 和 `/readyz` 占位入口，Worker 只完成配置校验和轮询占位；业务表和迁移将在后续步骤实现。
