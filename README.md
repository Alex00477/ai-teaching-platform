...
当前已建立可运行的前后端工程骨架，并完成认证、班级和作业发布基础模块；提交、批改和讲义业务仍按步骤实现。原始需求保留在 [需求规约.md](需求规约.md)，已确认的产品和工程取舍保留在 [decisions/](decisions/)。
...
当前 API 提供健康检查、`/auth` 认证入口、`/classes` 班级入口和 `/assignments` 作业创建/发布/查看入口；Worker 只完成配置校验和轮询占位，提交、批改和讲义业务迁移将在后续步骤实现。
